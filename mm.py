...
import pandas as pd
import math
from sklearn import cross_validation, linear_model, model_selection, neural_network, tree
import csv
import random
...
if __name__ == "__main__":
    ...
    tourney_data = pd.read_csv('TourneyDetailedResults.csv')
    ...
    model = VotingClassifier(
        estimators=[('NN', model1), ('KNN', model2), ('DT', model3), ('LR', model4), ('ADA', model5)], voting='soft')
    ...
    with open('readable-predictions.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerows(readable)
    with open('less-readable-predictions.csv', 'w') as f:
        writer = csv.writer(f)
